...
{{
  ...
  dat1<- within(dat1, {variable.1<- NULL})
  dat1<- within(dat1, {value.2<- NULL})
  ...
  predat3.1<- subset(dat1, select = c(variable, value, f1, h1))
  names(predat3.1)<-c("variable", "value", "f1", "Type")
  predat3.2<-subset(dat1, select=c(variable, value, f2, h2))
